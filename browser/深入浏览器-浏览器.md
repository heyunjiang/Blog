...
> 外链css 会阻塞 js 的执行，因为js需要获取节点的 css 信息，必须要等 render tree 生成之后，才能执行js。这也是为什么遇到 script 标签就是去渲染页面
...
浏览器js引擎维护的事件队列，是在js引擎空闲的时候执行。事件分为 `macrotask` 、 `microtask`

js引擎会先把 macrotask 中的任务执行完毕，才会去执行 microtask
...
1. 加载中：请求外链样式表、外链js时，发起http请求，此时 js 引擎会阻塞，空闲，从而执行任务队列
2. 运行中：所有js普通任务执行完毕，然后去执行任务队列

测试任务执行情况：setTimeout + promise + chrome
