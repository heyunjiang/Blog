...
**事件循环**：浏览器事件类型，包含 `macrotask` 、 `microtask`、`rerendertask`

macrotask: 普通 script 任务、dom event、setTimeout、setInterval
...
3. macrotask: dom event、setTimeout、setInterval
4. 重复上述循环
...
**任务队列**：保存异步任务的队列，分 macrotask 队列和 microtask 队列。

### 3.1 渲染引擎执行渲染任务，和js引擎执行call stack任务，2者有什么联系？
...
### 3.2 当浏览器渲染引擎去获取图片、css时，如果网速慢，然后就会造成主线程阻塞，此时如果调用栈中有任务，js引擎会去调用执行吗？
