...
在 uft-8 和 utf-16 这一类型的 utf 编码方案中，采用 unicode 字符集，经过不同的编码方案生成不同字节流，用于数据传输或保存；要展示对应编码方案保存的数据，则要对对应的字节流进入解码识别，然后绘制。其他的编码方案则是没有采用 unicode 字符集，比如 ASCII 、gbk 等，他们则是将编码方法与字符集都固定死了的。
...
答：它只是将中文转义了，依然采用的是 utf-16 编码。要说变化，也是 utf-16 -> utf-16
