...
> 本地库 -> 回滚(不保存内容) : git reset --hard <commitId>  
> 本地库 -> 暂存区(保存内容，撤销 commit，不撤销 add) : git reset --soft <commitId>
> 暂存区 -> 工作区(保存内容，撤销 commit, 撤销 add) : git reset HEAD <filename>
> 工作区 -> 放弃修改：git checkout -- <filename>
