...
主要 `createComponent` 创建一个 vnode 节点, 而 createComponent 又是通过 `new VNode` 来创建一个 vnode 节点

## 4 组件 update

在 vue 组件实例化的过程中，会为每个组件启动一个 watcher ，在 watcher 构造函数中，会执行如下操作

1. render 生成 vnode：调用 $createElement、VNode 等方法
...
vm._update: vnode -> vm.\_\_patch\_\_() -> createElm() | patchVnode() -> DOM
